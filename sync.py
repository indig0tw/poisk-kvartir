...
def push_new_ids(new_ids: set[str], token: str, attempts: int = 3) -> None:
    """Отправляет id, найденные локальным ботом и которых ещё нет в
    cloud_seen.json, в репозиторий - чтобы следующий облачный прогон их уже
    не переоткрывал и не слал то же уведомление ещё раз. Конфликт с
    параллельным коммитом с облачной стороны (та же гонка, что и обычно у
    двух независимых писателей) разрешается через sha текущего содержимого
    файла - GitHub API отклонит устаревший sha, и мы просто повторяем с
    актуальным."""
    if not new_ids:
        return
    ...
    for attempt in range(attempts):
        ...
        payload = {
            "message": "Синхронизация: объявления, найденные локальным ботом",
            "content": content_b64,
            "sha": sha,
        }

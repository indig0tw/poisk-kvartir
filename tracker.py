...
def _format_immoportal_message(city: City, listing: immoportal.Listing) -> str:
    price_line = f"{listing.kaltmiete:.0f} € Kaltmiete" if listing.kaltmiete is not None else "цена не распознана"
    size_line = f"{listing.wohnflaeche:.0f} м²" if listing.wohnflaeche is not None else "площадь не указана"
    return (
        f"🏠 Immoportal | {city.name} | {price_line} | {size_line}\n"
        f"Лимит по городу: {city.max_kaltmiete:.0f} €\n\n"
        f"{listing.title}\n{listing.url}"
    )
...
async def check_city_immoportal(client: httpx.AsyncClient, conn: sqlite3.Connection, city: City,
                                 max_wohnflaeche_qm: float, max_listings: int,
                                 bot_token: str, chat_id: str) -> None:
    search_url = immoportal.build_search_url(city.name)
    if search_url is None:
        return

    listings = await immoportal.fetch_listings(client, search_url, max_listings)

    for listing in listings:
        # Префикс "ip:" - по той же причине, что и у остальных источников:
        # id у Immoportal - число, но своя нумерация.
        seen_id = f"ip:{listing.ad_id}"
        if storage.is_seen(conn, seen_id):
            continue
        ...
        if matched:
            text = _format_immoportal_message(city, listing)
            await send_message(bot_token, chat_id, text)
            logger.info(f"[Immoportal/{city.name}] найдено подходящее объявление: {listing.title} - {listing.url}")

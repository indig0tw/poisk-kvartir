...
не повторилось незамеченным, каждый цикл `main.py` дополнительно проверяет
сырую выдачу Kleinanzeigen по трём городам (`_check_kleinanzeigen_health`);
если все три вернули 0 объявлений — это шлёт отдельное предупреждение в
Telegram (один раз, пока не восстановится) и пишет в лог с меткой
`[health]`. Работает только локально, встроено прямо в бесконечный цикл
бота — не зависит ни от какой сессии Claude Code и не ограничено по
времени, в отличие от cron-задач внутри сессии.

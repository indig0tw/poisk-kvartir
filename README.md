...
которые нашёл сам, но которых там ещё нет. Конфликт с параллельным
коммитом от самого GitHub Actions разрешается через `sha` содержимого
файла — API отклоняет устаревший `sha`, и `push_new_ids` просто повторяет
попытку с актуальным состоянием (до 3 раз).

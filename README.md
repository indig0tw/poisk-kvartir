...
[immowelt.de](https://www.immowelt.de/) и [quoka.de](https://www.quoka.de/) сразу по восьми
городам NRW (Mülheim an der Ruhr, Neuss, Köln, Düsseldorf, Essen, Oberhausen, Hilden,
Münster — Quoka без Mülheim an der Ruhr, см. ниже) и шлёт в Telegram новые
объявления, которые укладываются в лимиты по цене и площади, допустимые
джобцентром для одного человека.
...
- URL для фильтрации по городу — простой слаг в пути
  (`/liste/{slug}/wohnungen/mieten`), проверен вручную для всех 8 городов на
...
## Как работает поиск на Quoka

Quoka отдаёт список объявлений прямо в `<script type="application/ld+json">`
блоке (`@type: ItemList`) на странице поиска — не нужно ни парсить HTML
карточек, ни заходить на страницу объявления (см. `quoka.py`):

- `offers.price` в этом JSON — проверено вручную на реальных объявлениях,
  совпадает с «Kaltmiete» из текста описания, то есть надёжный источник
  цены (в отличие от Immowelt).
- Площадь сначала пробуем взять из структурированного `floorSize.value`,
  но у Quoka это поле часто просто `"0"`, даже когда площадь есть в
  тексте описания — тогда ищем «NN m²» / «NN qm» в описании через regex.
- **Важно**: JSON-LD от Quoka технически невалиден (буквальные переносы
  строк в значениях вместо `\n`) и не HTML-экранирован (`B&#252;rgergeld`
  вместо `Bürgergeld`) — парсим через `json.loads(..., strict=False)` и
  затем `html.unescape()` на title/description, иначе и парсинг падает, и
  фильтры по ключевым словам не находят то, что должны.
- **Mülheim an der Ruhr не подключён** — ни один вариант URL-слага не даёт
  корректную фильтрацию (сайт подсовывает соседние города вроде Oberhausen
  и Gelsenkirchen вместо настоящего Мюльхайма). Город и так покрыт
  Kleinanzeigen и Immowelt.

### Дополнительные фильтры (кроме Tauschwohnung)

Найдены при живом тестировании и реализованы в `filters.py`, применяются
ко всем трём источникам, где есть доступ к нужному тексту:
...
- **Bürgergeld-отказ** — некоторые арендодатели прямо пишут в описании
  ...
  Kleinanzeigen (текст описания со страницы объявления) и Quoka
  (описание есть прямо в JSON-LD) — для Immowelt недоступно из-за капчи на
  странице объявления.
...
все восемь городов на всех трёх сайтах и шлёт уведомления в чат с ботом.
...
- `quoka.py` — поиск объявлений на Quoka, разбор JSON-LD прямо из выдачи
  поиска (страница объявления не нужна вообще)
- `filters.py` — общие фильтры-исключения (Tauschwohnung, Monteurwohnung,
  встречный поиск, отказ от Bürgergeld) — используются `tracker.py` для
  всех источников
